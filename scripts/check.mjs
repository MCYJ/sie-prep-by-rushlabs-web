...
for (const file of htmlFiles) {
  ...
  for (const match of html.matchAll(/(?:src|href)="(\/sie-prep-by-rushlabs-web\/[^"?#]+)"/g)) {
    const pathname = match[1].replace("/sie-prep-by-rushlabs-web/", "");
...
if (htmlFiles.length !== 23) failures.push(`expected 23 HTML files, found ${htmlFiles.length}`);
if (failures.length) {
